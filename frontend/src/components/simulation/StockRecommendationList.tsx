...
export function StockRecommendationList({
  ...
  if (isLoading) {
    return (
      <Card className="h-full" data-testid="recommendation-loading-component">
  ...
  if (sortedRecommendations.length === 0) {
    return (
      <Card className="h-full" data-testid="recommendation-list">
        <div className="p-4 border-b border-gray-100">
          <h3 className="font-semibold text-gray-900">오늘의 추천 종목</h3>
        </div>
        <div className="p-8 flex flex-col items-center justify-center text-center h-64" data-testid="recommendation-empty">
  ...
  return (
    <Card className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto" data-testid="recommendation-list">
        <ul className="divide-y divide-gray-100">
          {sortedRecommendations.map((rec) => {
            ...
            return (
              <li key={rec.stock_code} data-stock-code={rec.stock_code}>
                <button
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-gray-900 truncate" data-testid="stock-name">
                          {rec.stock_name}
                        </span>
                        {rec.is_owned && (
                      ...
                      <span className="text-sm text-gray-500" data-testid="stock-code">{rec.stock_code}</span>
                    </div>
                    <span className={`px-2.5 py-1 rounded-lg border text-sm font-bold ${scoreColorClass}`} data-testid="confidence-score">
